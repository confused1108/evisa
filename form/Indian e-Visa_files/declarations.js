...
function errorifyQuestion(referId, result) {
    referId.css('display', '');
    referId.next().empty().addClass("error1").append(result);
    referId.addClass("error_input");
}
function correctifyQuestion(referId, result) {
    $(referId).val($(referId).val().replace(/^(&nbsp;|<br>)+/, ''));
    $(referId).val($.trim($(referId).val().replace(/\s{2,}/g, ' ')));
    referId.next().empty().removeClass("error1").append(result);
    referId.removeClass("error_input");
}

function errorifyRadio(referId, result) {

    referId.parent().next().find('.radioError').empty().addClass("error1").append(result);
    referId.addClass("error_input");
}
function correctifyRadio(referId, result) {

    referId.parent().next().find('.radioError').empty().addClass("error1").append(result);
    referId.addClass("error_input");
}

function correctify2(referId, result) {  
    referId.parent().next().empty().removeClass("error1").append(result);
    referId.removeClass("error_input");
}
...
function validate_Country(value, requiredOrNot) {
    ...
    if (requiredOrNot == 1 || (requiredOrNot == 0 && value.length >= 1)) {
        ...
        else if (alphabetRegex(value) == false)
...
function validate_Gender(value, requiredOrNot) {
    ...
    else if (requiredOrNot == 1 || value.length > 0)
    {
        if (alphabetRegex(value) == false || value.length != 1)
...
function validate_journeydate(value, requiredOrNot, visa_type_id) {
    ...
    if ((requiredOrNot == 1 || requiredOrNot == "1") || ((requiredOrNot == 0 || requiredOrNot == "0") && value.length >= 1)) {
        ...
        } else if ((visa_type_id == 3 || visa_type_id == "3") && dateLessThanYdays(value, 120) == false)
        {
            return "Date of Journey should not be greater than 120 days from Today's Date";
        }
...
function validate_conferencedate(value, requiredOrNot, visa_type_id) {
    requiredOrNot = parseInt(requiredOrNot);
    //visa_type_id = parseInt(visa_type_id);
    ...
    if ((requiredOrNot == 1 || requiredOrNot == "1") || ((requiredOrNot == 0 || requiredOrNot == "0") && value.length >= 1)) {
        ...
        } else if (myDate(value) == false)
        {
            ...
            return "Date of conference should be greater than or equals to today's date";
        } else if ((visa_type_id == 3 || visa_type_id == "3") && dateGreaterThanYdays(value, 4) == false) {
            return "Date of conference should be greater than 4 days from Today's Date";
        } else if ((visa_type_id == 3 || visa_type_id == "3") && dateLessThanYdays(value, 120) == false)
        {
            return "Date of conference should not be greater than 120 days from Today's Date";
        }
...
function validate_MaritalStatus(value, requiredOrNot) {
    ...
    if (requiredOrNot == 1 || (requiredOrNot == 0 && value.length >= 1)) {
        ...
        else if (value.length != 1 || IntRegex(value) == false)
            return "Invalid";
    }
    return "true";
}
function validate_occ_flag(value, requiredOrNot) {
    ...
    if (requiredOrNot == 1 || (requiredOrNot == 0 && value.length >= 1)) {
        ...
        if (value.length != 1 || alphabetRegex(value) == false)
            return "Invalid";
    }
    return "true";
}
...
function validate_country(value, requiredOrNot) {
    ...
    if (requiredOrNot == 1 || (requiredOrNot == 0 && value.length >= 1)) {
        ...
        {
            if (value.length != 3 || alphabetRegex(value) == false)
            {
                return "invalid country";
            }
        }
...
function validate_missionCode(value) {
    ...
    if (value.length != 4 || alphabetNumberRegex(value) == false) {
...
function validate_ppt_type_id(value) {
    ...
    if (value.length != 1 || IntRegex(value) == false) {
        return "Invalid passport type";
    }
...
function validate_email(value, requiredOrNot, visa_type_id) {
    ...
    requiredOrNot = parseInt(requiredOrNot);   
    if (requiredOrNot === 1 && (value === undefined || value.length === 0)) {
      
        return "Email-id is required";
    } else if (requiredOrNot == 1 || (requiredOrNot == 0 && (value !== undefined && value.length >= 1))) {
...
function validate_feedback_comments(value, requiredOrNot, maxlength) {
    requiredOrNot = parseInt(requiredOrNot);
